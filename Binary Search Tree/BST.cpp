...
int deleteHeap()
{
	int ROOT_DATA;
	int j;
	if(NUMBER_OF_HEAP == 0) window->output->Lines->Add("Empty!");
	else
	{
		ROOT_DATA = heap[1];
		heap[1] = heap[NUMBER_OF_HEAP];
		NUMBER_OF_HEAP--;
		int i = 1;

		while(i <= NUMBER_OF_HEAP/2)
		{
		...
		heap[i] = heap[NUMBER_OF_HEAP+1];
	}

	return ROOT_DATA;
}

void restore(int arr[], int root, int n)
{
	int small;
	while(root<=n/2)
	{
		small = arr[2*root+1]>arr[2*root] ? 2*root:2*root+1;
		if(arr[root]<arr[small]) break;
...
{
    output_line = "";
	int data_size = StrToInt(window->numOfNode->Text);
	for(int i=data_size/2;i>=1;i--)
		restore(heap, i, data_size);
	for(int i=data_size;i>=1;i--)
	{
		output_line += IntToStr(heap[i])+" -> ";
		heap[1] = heap[i];
		restore(heap, 1, i-1);
    }
...
void __fastcall Twindow::delHeapClick(TObject *Sender)
{
	int x = deleteHeap();
	output->Lines->Add( (String)x +" has been deleted!");
}
//---------------------------------------------------------------------------


void __fastcall Twindow::addRdmHeapClick(TObject *Sender)
{

	output_line = "Heap list(unsorted): ";
	int data_size = StrToInt(numOfNode->Text);
	int range_min = StrToInt(min->Text);
	int range_max = StrToInt(max->Text);
	...
	{
		while(NUMBER_OF_HEAP < data_size)
		{
			int h = rand() % (range_max-range_min+1) + range_min;
			insertHeap(h);
        }
		for(int i=1;i<data_size+1;i++)
		{
			output_line += String(heap[i]) + "->";
		}
...
void __fastcall Twindow::heap_Sort_btnClick(TObject *Sender)
{
	heapSort();
	output->Lines->Add("Heap Sort: ");
	output->Lines->Add(output_line);
	delete [] heap;
}
//---------------------------------------------------------------------------
